...
- **情绪标签 (21)**（定义在 `tags.py` 的 `EMOTION_TAGS`，按 valence 分组）：正向 = elation, amusement, enthusiasm, determination, pride, contentment, affection, relief, awe, longing；中性 = contemplation, confusion, surprise, arousal；负向 = anger, fear, disgust, bitterness, sadness, shame, helplessness
...
7. **Step 2 × N 轮**（默认 10）：
   ...
   - `03_tts_clone.py --post-prune --seed $((SEED + round*1000000))` 克隆（**每轮 seed 不同**，保证尾部被剪枝编号被复用时文本/参考仍不同；同轮内确定可续跑）
    - **`stop_tts()` 停止 TTS 释放 GPU** 给评估（⚠️ **不能只 `pkill -f "sgl-omni serve"`**：SGLang 用 `multiprocessing.spawn` 起的引擎子进程命令行是 `higgs_v3_env/bin/python3 -c from multiprocessing.spawn import spawn_main ...`，不含 "sgl-omni serve"，主进程被杀后它们被 init 收养(PPID=1)继续**每卡占 ~71GB 显存**。`stop_tts` 按 env 路径 `pkill -9 -f "${REPO_ROOT}/higgs_v3_env"` 整批杀掉——higgs_v3_env 为 TTS 独占故安全。另设 `trap 'stop_tts' EXIT` 作安全网：克隆失败/健康检查超时时 `set -e` 退出也会清理 TTS，防泄漏）
    - **评估顺序 SIM 先、CER 后**（SIM 快，先删一波缩小昂贵的 ASR 量）：SIM 评估 → **`pkill -f eval_sim.py` + `reap_orphans` 释放 GPU** → SIM 剪枝（`--max-cer 999` 只按 SIM<`MIN_SIM`(0.8 raw 余弦) 删）→ CER 评估（SIM 存活集；`--refresh-scan` 防缓存失效；`--batch-size 32 --audio-workers 16 --group-by-language --asr-max-new-tokens 512` 提速）→ **`pkill -f eval_cer.py` + `reap_orphans` 释放 GPU** → CER 剪枝（CER>0.03 或 SIM<0.8）。两次剪枝均 `--eval-source sidecar`。`START_STEP` 级别：`clone=1<sim=2<cer=3`（⚠️ SIM/CER 用**共享** conda env(omnivoice/qwen3-asr)，不能整批杀；`reap_orphans` 只回收 PPID==1 且命令行含 env-python 与 "multiprocessing" 的孤儿 worker，避免误伤本机其他任务）
   - 失败的评估/剪枝只 warning，继续下一轮
8. **最终**：`04_post_prune_stats.py` 最终统计 + `verify_kept_clones.py` 质量验证 + 各轮汇总

### 关键设计（README「关键设计」表）

- 每轮启停 TTS：克隆后释放 GPU 给 CER/SIM 评估；CER/SIM 各自结束后也 `pkill` 释放 GPU。**GPU 清理三工具**（`05_iterative_pipeline.sh`）：`stop_tts()`（按 `higgs_v3_env` 路径整批杀 TTS，含 spawn 引擎子进程）、`reap_orphans <env-marker>`（只回收 PPID==1 且含 "multiprocessing" 的孤儿 SIM/CER worker，共享 env 安全）、`trap 'stop_tts' EXIT`（异常退出兜底）。`03_launch_servers.sh` 启动前也按 `${CONDA_PYTHON%/bin/python3}` 补杀残留 TTS 引擎子进程（防上一轮泄漏导致本轮 OOM）
...
- **八卡生产**：`NUM_SERVERS=8`，`ALL_GPUS=0..7`，`WORKERS_PER_SERVER=32`（16→32 喂满 SGLang 连续批处理；`05_iterative_pipeline.sh` 内默认仍为 16），`TOTAL_ROUNDS=10`，`TOTAL_CLONE_HOURS=60000`（raw0.8 存活 ~10% + 目标 0.5h → gross≈58.5k），`SCAN_WORKERS=64`，`SIM_WORKERS=32`（每卡 4×8 卡），`PRUNE_WORKERS=32`
...
│   ├── run_eval_cer_parallel.sh        # 多分片并行 CER 启动脚本（--num-shards/--shard-index）
