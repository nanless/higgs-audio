...
from .tags import (
    HIGGS_TAG_RE, VALID_EMOTIONS, VALID_STYLES, VALID_SFX, VALID_PROSODY,
    SFX_TAGS, SFX_REQUIRES_ONO, count_tags,
)
...
def _is_complete_utterance(text: str) -> bool:
    ...
    if not clean:
        return False
    incomplete_endings = [",", ";", ":", "，", "；", "：", "-", "—", "..." , "…", "and", "or", "but", "还有", "可是", "因为", "而且"]
...
def quality_filter(texts: List[Dict], reject_severe_length_mismatch: bool = True,
                   max_tags_per_text: int = 5, max_same_tag_repeat: int = 2) -> List[Dict]:
    filtered = []
    for item in texts:
